...
```bash
node src/select-skill.mjs <cdn_json_path> data/shared_state/history.json
```
...
```bash
node src/download-skill.mjs <skill_id>
```
...
```bash
node src/md-to-png.mjs <report_path>
```

- 输出：同目录下的同名 `.png` 文件
- 默认使用 `./markdown.css` 样式
- 可指定自定义 CSS：`node src/md-to-png.mjs <report_path> <custom_css>`
...
```bash
node src/add-github-link.mjs <report_path>
```
...
```bash
node src/update-history.mjs data/shared_state/history.json <skill_id> <YYYY-MM-DD> <skill_link> <report_path>
```
